...
# out/scenarios/<name>/, so each scenario's data is self-contained and can be
# analyzed on its own afterwards without cross-referencing which records
# belong to which job. live/<node>/<node>.jsonl and
...
# out/ itself ends up holding only these per-scenario snapshots.
record_scenario() {
    local name="$1" job_id="$2" raw_output="$3"
    local dir="out/scenarios/$name"
    mkdir -p "$dir"
